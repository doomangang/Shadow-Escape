...
namespace ShadowEscape
{
    ...
    private CompletionUI _completionUI;
        ...
        {
            var systems = UnityObject.FindObjectsByType<EventSystem>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            var activeSystem = SelectPrimaryEventSystem(systems);
        ...
        private EventSystem SelectPrimaryEventSystem(EventSystem[] systems)
        {
            ...
            foreach (var system in systems)
            {
                ...
                {
                    if (HasUsableInputModule(system))
                    {
                        return system;
                    }
            ...
            return systems.FirstOrDefault(s => s != null);
        }
        ...
        private void EnsureEventSystemHasInputModule(EventSystem target)
        {
            if (target == null) return;

#if ENABLE_INPUT_SYSTEM
            var inputModule = target.GetComponent<InputSystemUIInputModule>()
                               ?? target.gameObject.AddComponent<InputSystemUIInputModule>();
            if (RuntimeUIInputActions.ConfigureModule(inputModule))
            {
                var legacy = target.GetComponent<StandaloneInputModule>();
                if (legacy != null)
                {
                    Destroy(legacy);
                }
            ...
            // Unable to configure Input System module (no actions). Remove and fall back.
            if (inputModule != null)
            {
                Destroy(inputModule);
            }
            ...
            {
                target.gameObject.AddComponent<StandaloneInputModule>();
            }
        }

        private bool HasUsableInputModule(EventSystem system)
        {
            if (system == null)
            {
                return false;
            ...
            return system.GetComponent<StandaloneInputModule>() != null;
        }
